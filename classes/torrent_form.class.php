...
class TORRENT_FORM {
	...
	function TORRENT_FORM($Torrent = false, $Error = false, $NewTorrent = true) {
		...
		global $UploadForm, $Categories, $Formats, $Qualities, $Bitrates, $Media, $TorrentID;
...
<?	}//function simple_form
    ...
    function movie_form($CategoryID) {
		$Torrent = $this->Torrent;
?>		<table cellpadding="3" cellspacing="1" border="0" class="layout border slice" width="100%">
			<tr id="name">
<?		if ($this->NewTorrent) {
			if ($this->Categories[$CategoryID] == 'Movies') { ?>
...
<?			} ?>
			...
			<tr id="year_tr">
                <td class="label">Year:</td>
				<td>
					<input type="text" id="year" name="year" size="5" value="<?=display_str($Torrent['Year']) ?>" /> This is the year of the original release.
				</td>
			</tr>
			<tr>
				...
				<td>
					<select id="format" name="format" onchange="Format()">
						<option>---</option>
<?
		foreach (Misc::display_array($this->Qualities) as $Format) {
			echo "\t\t\t\t\t\t<option value=\"$Format\"";
			if ($Format == $Torrent['Format']) {
				echo ' selected="selected"';
			}
